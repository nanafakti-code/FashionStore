-- ENABLE_GUEST_DB_CART.sql

-- 1. Modify cart_items to support guests
ALTER TABLE public.cart_items ALTER COLUMN user_id DROP NOT NULL;
ALTER TABLE public.cart_items ADD COLUMN IF NOT EXISTS session_id TEXT DEFAULT NULL;
CREATE INDEX IF NOT EXISTS idx_cart_items_session_id ON public.cart_items(session_id);

-- 2. Update add_to_cart_v2 to handle session_id
...
BEGIN
  ...
  -- 1. Validation and Stock Check (Same as before)
...
BEGIN
  RETURN QUERY
  SELECT 
    ci.id, ci.product_id, ci.variante_id, ci.quantity, ci.talla, ci.color, ci.precio_unitario, ci.created_at, ci.expires_at,
...
-- 4. Function to remove from guest cart
